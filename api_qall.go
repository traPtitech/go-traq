...
type QallApiGetEndpointsRequest struct {
	ctx        context.Context
	ApiService *QallApiService
}

func (r QallApiGetEndpointsRequest) Execute() (*QallEndpointResponse, *http.Response, error) {
	return r.ApiService.GetEndpointsExecute(r)
}

/*
GetEndpoints LiveKitエンドポイントを取得

接続可能なLiveKitエンドポイントを取得します。

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@return QallApiGetEndpointsRequest
*/
func (a *QallApiService) GetEndpoints(ctx context.Context) QallApiGetEndpointsRequest {
	return QallApiGetEndpointsRequest{
		ApiService: a,
		ctx:        ctx,
	}
...
func (a *QallApiService) GetEndpointsExecute(r QallApiGetEndpointsRequest) (*QallEndpointResponse, *http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "QallApiService.GetEndpoints")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}
