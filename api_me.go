...
type MeAPIGetMyStampRecommendationsRequest struct {
	ctx        context.Context
	ApiService *MeAPIService
	limit      *int32
}
...
// 件数
func (r MeAPIGetMyStampRecommendationsRequest) Limit(limit int32) MeAPIGetMyStampRecommendationsRequest {
	r.limit = &limit
	return r
}

func (r MeAPIGetMyStampRecommendationsRequest) Execute() ([]GetMyStampRecommendations200ResponseInner, *http.Response, error) {
	return r.ApiService.GetMyStampRecommendationsExecute(r)
}

/*
GetMyStampRecommendations スタンプレコメンドを取得
...
スタンプを使用したことがないユーザーの場合は空配列が返されます。
	...
	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@return MeAPIGetMyStampRecommendationsRequest
*/
func (a *MeAPIService) GetMyStampRecommendations(ctx context.Context) MeAPIGetMyStampRecommendationsRequest {
	return MeAPIGetMyStampRecommendationsRequest{
		ApiService: a,
		ctx:        ctx,
	}
...
func (a *MeAPIService) GetMyStampRecommendationsExecute(r MeAPIGetMyStampRecommendationsRequest) ([]GetMyStampRecommendations200ResponseInner, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodGet
		localVarPostBody    interface{}
		formFiles           []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "MeAPIService.GetMyStampRecommendations")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/users/me/stamp-recommendations"
